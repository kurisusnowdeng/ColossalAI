...
def build_model_optimizer(args, model_args):
    ...
    # optimizer
    optimizer = GPT.configure_optimizers(model,
                                         args.optim,
                                         args.lr,
                                         args.decay,
                                         zero=args.zero_stage == 1,
                                         **args.optim_args)
...
amp_ctx = nullcontext() if args.dtype == torch.float32 else torch.amp.autocast(device_type=args.device,
                                                                               dtype=args.dtype)
...
while True:
    ...
    if iter_num % args.eval_interval == 0:
        ...
        if loss < best_val_loss:
            best_val_loss = loss
            if iter_num > 0 and args.out_path is not None:
                checkpoint = {
