...
def shard(module, config):
    ...
    if get_world_size() > 1 and config.fsdp:
        return FSDP(
            module,
...
class GPT(nn.Module):
    ...
    def require_backward_grad_sync(self, sync):
        if isinstance(self.lm, DDP):
            self.lm.require_backward_grad_sync = sync
    ...
    def configure_optimizers(self, algorithm, learning_rate, weight_decay, *args, **kwargs):
        ...
        optimizer = getattr(torch.optim, algorithm)(optim_groups, lr=learning_rate, *args, **kwargs)
        return optimizer
